/* ----------------------------------------------------------------------------
 * This file was automatically generated by SWIG (http://www.swig.org).
 * Version 1.3.40
 *
 * Do not make changes to this file unless you know what you are doing--modify
 * the SWIG interface file instead.
 * ----------------------------------------------------------------------------- */

namespace BWAPI {

using System;
using System.Runtime.InteropServices;

public partial class UpgradeTypeSet : IDisposable 
#if !SWIG_DOTNET_1
    , System.Collections.Generic.ICollection<UpgradeType>
#endif
 {
  private HandleRef swigCPtr;
  protected bool swigCMemOwn;

  internal UpgradeTypeSet(IntPtr cPtr, bool cMemoryOwn) {
    swigCMemOwn = cMemoryOwn;
    swigCPtr = new HandleRef(this, cPtr);
  }

  internal static HandleRef getCPtr(UpgradeTypeSet obj) {
    return (obj == null) ? new HandleRef(null, IntPtr.Zero) : obj.swigCPtr;
  }

  ~UpgradeTypeSet() {
    Dispose();
  }

  public virtual void Dispose() {
    lock(this) {
      if (swigCPtr.Handle != IntPtr.Zero) {
        if (swigCMemOwn) {
          swigCMemOwn = false;
          bridgePINVOKEProxy.delete_UpgradeTypeSet(swigCPtr);
        }
        swigCPtr = new HandleRef(null, IntPtr.Zero);
      }
      GC.SuppressFinalize(this);
    }
  }


  
  public int Count {
    get {
      return (int)size();
    }
  }

  public bool IsReadOnly {
    get { 
      return false; 
    }
  }

#if !SWIG_DOTNET_1
 public System.Collections.Generic.ICollection<UpgradeType> Values {
    get {
      System.Collections.Generic.ICollection<UpgradeType> values = new System.Collections.Generic.List<UpgradeType>();
      IntPtr iter = create_iterator_begin();
      try {
        while (true) {
          values.Add(get_next_key(iter));
        }
      } catch (ArgumentOutOfRangeException) {
      }
      return values;
    }
  }
 
  public bool Contains(UpgradeType item) {
    if ( ContainsKey(item)) {
      return true;
    } else {
      return false;
    }
  }

  public void CopyTo(UpgradeType[] array) {
    CopyTo(array, 0);
  }

  public void CopyTo( UpgradeType[] array, int arrayIndex) {
    if (array == null)
      throw new ArgumentNullException("array");
    if (arrayIndex < 0)
      throw new ArgumentOutOfRangeException("arrayIndex", "Value is less than zero");
    if (array.Rank > 1)
      throw new ArgumentException("Multi dimensional array.", "array");
    if (arrayIndex+this.Count > array.Length)
      throw new ArgumentException("Number of elements to copy is too large.");

   System.Collections.Generic.IList<UpgradeType> keyList = new System.Collections.Generic.List<UpgradeType>(this.Values);
    for (int i = 0; i < this.Count; i++) {
      UpgradeType currentKey = keyList[i];
      array.SetValue( currentKey, arrayIndex+i);
    }
  }

  System.Collections.Generic.IEnumerator< UpgradeType> System.Collections.Generic.IEnumerable<UpgradeType>.GetEnumerator() {
    return new UpgradeTypeSetEnumerator(this);
  }

  System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator() {
    return new UpgradeTypeSetEnumerator(this);
  }

  public UpgradeTypeSetEnumerator GetEnumerator() {
    return new UpgradeTypeSetEnumerator(this);
  }

  // Type-safe enumerator
  /// Note that the IEnumerator documentation requires an InvalidOperationException to be thrown
  /// whenever the collection is modified. This has been done for changes in the size of the
  /// collection but not when one of the elements of the collection is modified as it is a bit
  /// tricky to detect unmanaged code that modifies the collection under our feet.
  public sealed class UpgradeTypeSetEnumerator : System.Collections.IEnumerator, 
      System.Collections.Generic.IEnumerator< UpgradeType>
  {
    private UpgradeTypeSet collectionRef;
    private System.Collections.Generic.IList<UpgradeType> keyCollection;
    private int currentIndex;
    private object currentObject;
    private int currentSize;

    public UpgradeTypeSetEnumerator(UpgradeTypeSet collection) {
      collectionRef = collection;
      keyCollection = new System.Collections.Generic.List<UpgradeType>(collection.Values);
      currentIndex = -1;
      currentObject = null;
      currentSize = collectionRef.Count;
    }

    // Type-safe iterator Current
    public  UpgradeType Current {
      get {
        if (currentIndex == -1)
          throw new InvalidOperationException("Enumeration not started.");
        if (currentIndex > currentSize - 1)
          throw new InvalidOperationException("Enumeration finished.");
        if (currentObject == null)
          throw new InvalidOperationException("Collection modified.");
        return ( UpgradeType)currentObject;
      }
    }

    // Type-unsafe IEnumerator.Current
    object System.Collections.IEnumerator.Current {
      get {
        return Current;
      }
    }

    public bool MoveNext() {
      int size = collectionRef.Count;
      bool moveOkay = (currentIndex+1 < size) && (size == currentSize);
      if (moveOkay) {
        currentIndex++;
        UpgradeType currentKey = keyCollection[currentIndex];
        currentObject = currentKey;
      } else {
        currentObject = null;
      }
      return moveOkay;
    }

    public void Reset() {
      currentIndex = -1;
      currentObject = null;
      if (collectionRef.Count != currentSize) {
        throw new InvalidOperationException("Collection modified.");
      }
    }

    public void Dispose() {
      currentIndex = -1;
      currentObject = null;
    }
  }
#endif
  

  public UpgradeTypeSet() : this(bridgePINVOKEProxy.new_UpgradeTypeSet__SWIG_0(), true) {
  }

  public UpgradeTypeSet(UpgradeTypeSet other) : this(bridgePINVOKEProxy.new_UpgradeTypeSet__SWIG_1(UpgradeTypeSet.getCPtr(other)), true) {
    if (bridgePINVOKEProxy.SWIGPendingException.Pending) throw bridgePINVOKEProxy.SWIGPendingException.Retrieve();
  }

  private uint size() {
    uint ret = bridgePINVOKEProxy.UpgradeTypeSet_size(swigCPtr);
    return ret;
  }

  public bool empty() {
    bool ret = bridgePINVOKEProxy.UpgradeTypeSet_empty(swigCPtr);
    return ret;
  }

  public void Clear() {
    bridgePINVOKEProxy.UpgradeTypeSet_Clear(swigCPtr);
  }

  public UpgradeType getitem(UpgradeType key) {
    UpgradeType ret = new UpgradeType(bridgePINVOKEProxy.UpgradeTypeSet_getitem(swigCPtr, UpgradeType.getCPtr(key)), false);
    if (bridgePINVOKEProxy.SWIGPendingException.Pending) throw bridgePINVOKEProxy.SWIGPendingException.Retrieve();
    return ret;
  }

  public bool ContainsKey(UpgradeType key) {
    bool ret = bridgePINVOKEProxy.UpgradeTypeSet_ContainsKey(swigCPtr, UpgradeType.getCPtr(key));
    if (bridgePINVOKEProxy.SWIGPendingException.Pending) throw bridgePINVOKEProxy.SWIGPendingException.Retrieve();
    return ret;
  }

  public void Add(UpgradeType key) {
    bridgePINVOKEProxy.UpgradeTypeSet_Add(swigCPtr, UpgradeType.getCPtr(key));
    if (bridgePINVOKEProxy.SWIGPendingException.Pending) throw bridgePINVOKEProxy.SWIGPendingException.Retrieve();
  }

  public bool Remove(UpgradeType key) {
    bool ret = bridgePINVOKEProxy.UpgradeTypeSet_Remove(swigCPtr, UpgradeType.getCPtr(key));
    if (bridgePINVOKEProxy.SWIGPendingException.Pending) throw bridgePINVOKEProxy.SWIGPendingException.Retrieve();
    return ret;
  }

  public IntPtr create_iterator_begin() {
    IntPtr ret = bridgePINVOKEProxy.UpgradeTypeSet_create_iterator_begin(swigCPtr);
    return ret;
  }

  public UpgradeType get_next_key(IntPtr swigiterator) {
    UpgradeType ret = new UpgradeType(bridgePINVOKEProxy.UpgradeTypeSet_get_next_key(swigCPtr, swigiterator), false);
    if (bridgePINVOKEProxy.SWIGPendingException.Pending) throw bridgePINVOKEProxy.SWIGPendingException.Retrieve();
    return ret;
  }

}

}
